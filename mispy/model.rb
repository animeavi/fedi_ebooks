...
class Model
  ...
  def consume(path)
    ...
    when "csv"
      ...
      log("Reading plaintext corpus from #{path} (if this is a json or csv file, "\
          "please rename the file with an extension and reconsume)")
  ...
  def reconstruct(tikis)
    ...
    tikis.each do |tiki|
      ...
      text += " " if last_token && NLP.space_between?(last_token, token)
      text += token
      last_token = token
    end
  ...
  # @param bigrams [Hash]
  # @return [Array<Integer>]
  def generate_respoding(responding = false, passes = 5, n = :unigrams, sentences = nil, unigrams = nil, bigrams = nil)
    if responding
  ...
  def find_relevant_tokens(input)
    ...
    tokenized.each do |token|
      ...
      if token_id_db != -1
        relevant << token_id_db unless NLP.stopword?(token)
